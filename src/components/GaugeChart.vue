<template>
  <div class="w-48 h-32">
    <canvas ref="gaugeChart" style="pointer-events: none"></canvas>
  </div>
</template>
...
watch(
  () => props.gaugeValue,
  (value) => {
    gaugeChartData.datasets[0].needleValue = value
  }
)
...
const borderColors = computed(() => {
  return segments.value.map((segment) => segment.borderColor)
})
...
const gaugeValueIndex = computed(() => {
  ...
  for (let i = 0; i < dataSegments.value.length; i++) {
    ...
    if (cumulativeSum >= props.gaugeValue) {
      return i
    }
  }
})
